...
def parse_args():
    ...
    parser.add_argument("--output", "-o",
        help="Folder for las outpout",
        default="test/data/")
    parser.add_argument("--tiles_size", '-s', type=int,
        help="Size (in meter) for the output tiles",
        default=10)
    parser.add_argument("--keep", "-k", type=int,
        help="Number of tiles to keep (out of 9). Default: all",
        default=9)
    parser.add_argument("--offset", "-d", nargs=2, type=int,
...
def main(args):
    ...
    assert np.all(bounds[1] - bounds[0] > (np.array(args.offset) + 3 * args.tiles_size)), \
