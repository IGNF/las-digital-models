...
def run_extract_minz_from_raster_one_tile(config: DictConfig):
    ...
    """
    tilename_raster, _ = os.path.splitext(config.extract_stat.input_raster_filename)
    input_raster = os.path.join(
        config.extract_stat.input_raster_dir, f"{tilename_raster}.tif"
    )  # path to the RASTER file
    if not os.path.isfile(input_raster):
        raise ValueError(f"Input raster file not found: {input_raster}")

    tilename_geom, _ = os.path.splitext(config.extract_stat.input_geometry_filename)
    input_geometry = os.path.join(
        config.extract_stat.input_geometry_dir, f"{tilename_geom}.shp"
    ...
    # for export
    crs = config.extract_stat.spatial_reference
    geometry_filename = f"{tilename_raster}_lines.GeoJSON"
    output_geometry = os.path.join(config.extract_stat.output_dir, geometry_filename)
